...
int *backward(Matrix *input, Model *model, Matrix *reference, double learning_rate)
{
    ...
    for (int i = L - 1; i >= 1; i--)
    {
        step(model->layers[i + 1].delta, model->layers[i + 1].weights, model->layers[i - 1].activations, &model->layers[i], model->batch_size, learning_rate);
        // printf("layer %d done\n",(i+1));
    }
    step(model->layers[1].delta, model->layers[1].weights, input, &model->layers[0], model->batch_size, learning_rate);
